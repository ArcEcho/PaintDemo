...
void SPaintDemoRuler::Construct(const SPaintDemoRuler::FArguments& InArgs)
{
...
float SPaintDemoRuler::DetermineOptimalSpacing(float InPixelsPerInput, uint32 MinTick, float MinTickSpacing) const
{
...
void SPaintDemoRuler::SetRuling(FVector2D InAbsoluteOrigin, float InSlateToUnitScale)
{
    AbsoluteOrigin = InAbsoluteOrigin;
    SlateToUnitScale = InSlateToUnitScale;
}

void SPaintDemoRuler::SetCursor(TOptional<FVector2D> InAbsoluteCursor)
{
    AbsoluteCursor = InAbsoluteCursor;
}

int32 SPaintDemoRuler::DrawTicks(FSlateWindowElementList& OutDrawElements, const struct FScrubRangeToScreen& RangeToScreen, FDrawTickArgs& InArgs) const
{
...
int32 SPaintDemoRuler::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyClippingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
...
FReply SPaintDemoRuler::OnMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
FReply SPaintDemoRuler::OnMouseButtonUp(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
    return FReply::Unhandled();
}

FReply SPaintDemoRuler::OnMouseMove(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
    return FReply::Unhandled();
}

FVector2D SPaintDemoRuler::ComputeDesiredSize(float) const
{
    return Orientation == Orient_Horizontal ? FVector2D(100, 18) : FVector2D(18, 100);
}

FReply SPaintDemoRuler::OnMouseWheel(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
    return FReply::Unhandled();
}
